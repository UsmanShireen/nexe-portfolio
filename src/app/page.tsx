...
export default function Home() {
  return (
    <main className="min-h-screen bg-[#050508] text-white font-sans overflow-x-hidden">
      ...
      {/* NAV */}
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 sm:px-8 py-3 sm:py-4 bg-[#050508]/80 backdrop-blur-xl border-b border-white/5">
        <div className="font-black text-sm sm:text-xl tracking-[3px] sm:tracking-[6px] uppercase bg-gradient-to-r from-purple-400 to-cyan-400 bg-clip-text text-transparent">
          Usman Shireen
        </div>
        <div className="flex items-center gap-2 sm:gap-4">
          <Link
            ...
            target="_blank"
            className="hidden sm:flex items-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 px-4 py-2 rounded-full text-xs font-mono tracking-wider text-white/50 hover:text-white transition-all duration-300"
          >
            <GithubIcon />
            GitHub
          </Link>
          ...
          {/* Mobile icons only */}
          <Link href="https://github.com/UsmanShireen" target="_blank"
            className="sm:hidden flex items-center justify-center w-8 h-8 bg-white/5 border border-white/10 rounded-full text-white/50">
            <GithubIcon />
          </Link>
          <Link href="https://www.linkedin.com/in/usman-shireen-4211262b7/" target="_blank"
            className="sm:hidden flex items-center justify-center w-8 h-8 bg-white/5 border border-white/10 rounded-full text-white/50">
            <LinkedinIcon />
          </Link>
          <span className="bg-emerald-400/10 border border-emerald-400/30 text-emerald-400 px-2 sm:px-3 py-1 rounded-full font-mono text-[9px] sm:text-[10px] tracking-[1px] sm:tracking-[2px] whitespace-nowrap">
            ● 6 LIVE
          </span>
        </div>
      </nav>
      ...
      {/* HERO */}
      <section className="relative z-10 min-h-screen flex flex-col items-center justify-center text-center px-4 sm:px-6 pt-24 pb-20">
        <div className="flex items-center gap-2 sm:gap-3 mb-5 sm:mb-6">
          <div className="w-6 sm:w-10 h-px bg-white/20" />
          <span className="font-mono text-[9px] sm:text-[11px] tracking-[2px] sm:tracking-[4px] text-white/30 uppercase">
            Nexe-Agent Internship 2026
          </span>
          <div className="w-6 sm:w-10 h-px bg-white/20" />
        </div>

        <h1 className="font-black text-[clamp(40px,10vw,120px)] leading-[0.9] tracking-tight mb-2 uppercase">
        ...
        <p className="text-white/40 max-w-xs sm:max-w-lg text-sm sm:text-base leading-relaxed mt-4 sm:mt-6 mb-8 sm:mb-10">
          6 production-grade AI agent projects — from beginner tool-calling to
          advanced multi-agent systems with communication layers.
        </p>

        {/* Stats */}
        <div className="flex gap-5 sm:gap-16 mb-10 sm:mb-12">
          {[
            ...
            <div key={s.label} className="text-center">
              <div className="font-black text-3xl sm:text-5xl bg-gradient-to-b from-purple-400 to-cyan-400 bg-clip-text text-transparent leading-none">
                {s.num}
              </div>
              <div className="font-mono text-[8px] sm:text-[10px] tracking-[1px] sm:tracking-[2px] text-white/30 uppercase mt-1 sm:mt-2">
                {s.label}
              </div>
            </div>
          ))}
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto px-4 sm:px-0">         
          <a  href="#projects"
            className="w-full sm:w-auto group flex items-center justify-center gap-3 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 px-6 py-3 rounded-full text-sm font-semibold tracking-wider transition-all duration-300">
            VIEW ALL PROJECTS
          ...
          <Link
            ...
            target="_blank"
            className="w-full sm:w-auto group flex items-center justify-center gap-3 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 px-6 py-3 rounded-full text-sm font-semibold tracking-wider transition-all duration-300 text-white/60 hover:text-white"
          >
            <GithubIcon />
            VIEW ON GITHUB
          </Link>
      ...
      {/* PROJECTS */}
      <section id="projects" className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 py-16 sm:py-24">
        <div className="flex items-center gap-4 mb-3">
          <span className="font-mono text-[10px] tracking-[3px] sm:tracking-[4px] text-white/30 uppercase">// LIVE PROJECTS</span>
          <div className="h-px flex-1 bg-white/5 max-w-48" />
        </div>
        <h2 className="font-black text-[clamp(30px,5vw,64px)] uppercase leading-none mb-10 sm:mb-14">
          ALL{" "}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
          {projects.map((p) => (
            <div
              key={p.id}
              className={`group relative bg-[#0c0c14] border border-white/[0.06] rounded-2xl p-5 sm:p-7 flex flex-col transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl ${p.glow} ${p.border} overflow-hidden`}
            >
              {/* Top color line */}
              <div
              ...
              <div
                ...
                style={{ background: `radial-gradient(circle at 50% 0%, ${p.color}12 0%, transparent 60%)` }}
              />
              ...
              <div className="flex items-center justify-between mb-4 sm:mb-5 relative z-10">
                ...
                <span className={`text-[10px] font-mono tracking-[1px] px-2 sm:px-3 py-1 rounded-full border ${p.levelColor}`}>
                  {p.level.toUpperCase()}
                </span>
              </div>

              <span className="text-3xl sm:text-4xl mb-3 sm:mb-4 relative z-10 block">{p.icon}</span>
              <h3 className="text-base sm:text-lg font-bold mb-2 relative z-10">{p.title}</h3>
              <p className="text-xs sm:text-sm text-white/40 leading-relaxed mb-4 sm:mb-5 flex-1 relative z-10">{p.desc}</p>

              {/* Tags */}
              <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-4 sm:mb-5 relative z-10">
                {p.tags.map((tag) => (
                  <span key={tag} className="text-[10px] sm:text-[11px] font-mono bg-white/5 border border-white/[0.06] text-white/30 px-2 sm:px-2.5 py-1 rounded-md">
                    {tag}
                  </span>
                ))}
              </div>
              ...
              <div className="flex gap-2 sm:gap-3 relative z-10">
                <Link
                  href={p.url}
                  target="_blank"
                  className={`flex-1 flex items-center justify-between ${p.btnColor} text-xs sm:text-sm font-bold tracking-wider uppercase px-3 sm:px-4 py-2.5 sm:py-3 rounded-xl transition-all duration-200`}
                >
                  LIVE APP
                ...
                <Link
                  ...
                  className="flex items-center justify-center w-10 sm:w-12 h-10 sm:h-12 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 rounded-xl transition-all duration-200 text-white/40 hover:text-white"
                >
                  <GithubIcon />
                </Link>
              </div>
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 py-16 sm:py-24">
        <div className="flex items-center gap-4 mb-3">
          <span className="font-mono text-[10px] tracking-[3px] sm:tracking-[4px] text-white/30 uppercase">// TECHNOLOGIES</span>
          <div className="h-px flex-1 bg-white/5 max-w-48" />
        </div>
        <h2 className="font-black text-[clamp(30px,5vw,64px)] uppercase leading-none mb-10 sm:mb-14">
          TECH{" "}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4">
          {techStack.map((t) => (
            <div
              key={t.name}
              className="bg-[#0c0c14] border border-white/[0.06] hover:border-white/[0.12] rounded-xl sm:rounded-2xl p-4 sm:p-5 flex items-center gap-3 sm:gap-4 transition-all duration-300 group"
            >
              <span className="text-xl sm:text-2xl">{t.icon}</span>
              <div>
                <div className="text-xs sm:text-sm font-semibold text-white group-hover:text-white/90">{t.name}</div>
                <div className="text-[10px] sm:text-xs text-white/30 mt-0.5">{t.desc}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-16">
        <div className="bg-[#0c0c14] border border-white/[0.06] rounded-2xl sm:rounded-3xl p-8 sm:p-12 flex flex-col items-center text-center sm:text-left sm:flex-row sm:items-center justify-between gap-6 sm:gap-8">
          <div>
            <div className="font-mono text-[10px] tracking-[4px] text-white/30 uppercase mb-3">
              // SOURCE CODE
            </div>
            <h3 className="font-black text-2xl sm:text-3xl uppercase mb-3">
              VIEW ON{" "}
              <span className="bg-gradient-to-r from-purple-400 to-cyan-400 bg-clip-text text-transparent">
                GITHUB
              </span>
          ...
          <Link
            href="https://github.com/UsmanShireen/nexe-agent-projects"
            target="_blank"
            className="w-full sm:w-auto flex items-center justify-center gap-3 bg-white text-black font-bold px-6 sm:px-8 py-3 sm:py-4 rounded-xl sm:rounded-2xl hover:bg-white/90 transition-all duration-300 text-sm tracking-wider whitespace-nowrap"
          >
            <GithubIcon />
            STAR ON GITHUB →
          </Link>
      ...
      {/* FOOTER */}
      <footer className="relative z-10 border-t border-white/5 px-4 sm:px-8 py-8 sm:py-10 flex flex-col sm:flex-row items-center justify-between gap-6">
        <div className="text-center sm:text-left">
          <div className="font-black text-lg sm:text-xl tracking-[4px] sm:tracking-[6px] uppercase bg-gradient-to-r from-purple-400 to-cyan-400 bg-clip-text text-transparent mb-1">
            NEXE-AGENT
          </div>
          <p className="text-xs text-white/30">Agentic AI Developer Internship — 2026</p>
        </div>
        <div className="text-center sm:text-right font-mono text-xs text-purple-400 space-y-2">
          <div>
